...
public class PostImageServiceImpl implements PostImageService {
    ...
    public PostImage uploadPostImageWithHashtags(List<MultipartFile> files, Long postId) {
        ...
        // 4. AI 해시태그 생성
        List<Hashtag> extractedHashtags = null; // 첫 번째 URL 사용
        try {
            extractedHashtags = hashtagService.createHashtagsFromUrls(imageUrls);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        // 5. PostImageHashtag 저장
        for (Hashtag tag : extractedHashtags) {
            Hashtag hashtag = hashtagService.createOrGet(tag.getName());
